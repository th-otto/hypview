                       ...
                       const char  *font_name)
{
    ...
    for (i = 0; i < list->count; i++) {
        ...
        if (strcmp (font_name, def->base_font) == 0) {
...
HPDF_EXPORT(HPDF_STATUS)
HPDF_AddIntent(HPDF_Doc  pdf, HPDF_OutputIntent  intent)
{
    HPDF_Array intents;
    if (!HPDF_HasDoc (pdf))
        return HPDF_INVALID_DOCUMENT;
...
HPDF_AddColorspaceFromProfile  (HPDF_Doc pdf, HPDF_Dict icc)
{
    HPDF_STATUS ret = HPDF_OK;
    HPDF_Array iccentry;
             ...
             void                 *user_data)
{
    ...
    memset (pdf, 0, sizeof (HPDF_Doc_Rec));
...
HPDF_NewDoc  (HPDF_Doc  pdf)
{
    ...
    strcpy (ptr, "Haru Free PDF Library ");
    ptr += sizeof("Haru Free PDF Library ") - 1;
                    ...
                    const char  *encoding_name)
{
    ...
    for (i = 0; i < pdf->font_mgr->count; i++) {
        ...
        if (strcmp (attr->fontdef->base_font, font_name) == 0 &&
                strcmp (attr->encoder->name, encoding_name) == 0)
            return font;
    }
