	...
	for (i = 0; i < len; i++)
	{
		...
		} else
		{
			...
			if (last_btype == HPDF_BYTE_TYPE_TRIAL ||
					(btype == HPDF_BYTE_TYPE_LEAD && last_btype == HPDF_BYTE_TYPE_SINGLE))
			{
...
static HPDF_Dict CreateCMap(HPDF_Encoder encoder, HPDF_Xref xref)
{
	...
	*pbuf++ = ' ';
	...
	{
		HPDF_CidRange_Rec *range = (HPDF_CidRange_Rec *) HPDF_List_ItemAt(attr->code_space_range, i);
	...
	pbuf = HPDF_StrCpy(pbuf, "CMapName currentdict /CMap " "defineresource pop\r\n", eptr);
...
HPDF_Font HPDF_Type0Font_New(HPDF_MMgr mmgr, HPDF_FontDef fontdef, HPDF_Encoder encoder, HPDF_Xref xref)
{
	...
	memset(attr, 0, sizeof(HPDF_FontAttr_Rec));
