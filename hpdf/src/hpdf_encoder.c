...
static const HPDF_BuiltinEncodingData *HPDF_BasicEncoder_FindBuiltinData(const char *encoding_name)
{
	HPDF_UINT i = 0;
	...
	{
		if (strcmp(HPDF_BUILTIN_ENCODINGS[i].encoding_name, encoding_name) == 0)
			break;
...
HPDF_Encoder HPDF_BasicEncoder_New(HPDF_MMgr mmgr, const char *encoding_name)
{
	...
	memset(encoder_attr, 0, sizeof(HPDF_BasicEncoderAttr_Rec));
...
HPDF_STATUS HPDF_CMapEncoder_InitAttr(HPDF_Encoder encoder)
{
	...
	encoder_attr = (HPDF_CMapEncoderAttr) HPDF_GetMem(encoder->mmgr, sizeof(HPDF_CMapEncoderAttr_Rec));

	if (!encoder_attr)
		return encoder->error->error_no;

	memset(encoder_attr, 0, sizeof(HPDF_CMapEncoderAttr_Rec));
	encoder->attr = encoder_attr;
