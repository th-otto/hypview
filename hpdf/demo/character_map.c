...
static void draw_page(HPDF_Doc pdf, HPDF_Page page, HPDF_Font title_font, HPDF_Font font, HPDF_BYTE h_byte, HPDF_BYTE l_byte)
{
	...
	for (xpos = 0; xpos <= 17; xpos++)
	{
		...
		if (xpos > 0 && xpos <= 16)
		{
			...
			w = HPDF_Page_TextWidth(page, (char *) buf);
			HPDF_Page_BeginText(page);
			HPDF_Page_MoveTextPos(page, x + ((double) 20 - w) / 2, h_count * CELL_HEIGHT + 45);
			HPDF_Page_ShowText(page, (char *) buf);
			HPDF_Page_EndText(page);
		}
	}
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i <= 255; i++)
	{
		if (flg[i])
		{
			...
			sprintf(buf, "0x%04X-0x%04X", (unsigned int) (i * 256 + min_l), (unsigned int) (i * 256 + max_l));
			...
			sprintf(buf, "%s (%s) 0x%04X-0x%04X", argv[1], argv[2],
					 (unsigned int) (i * 256 + min_l), (unsigned int) (i * 256 + max_l));
			HPDF_Page_SetFontAndSize(page, title_font, 10);
			HPDF_Page_BeginText(page);
