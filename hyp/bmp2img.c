...
char const gl_program_name[] = "bmp2img";
...
static gboolean conv_file(const char *filename)
{
	...
	pic.pi_compressed = 1;
	pic_calcsize(&pic);
	headerlen = img_header(&buf, &pic);
	datalen = img_pack(buf, dest, &pic);
	pic.pi_datasize = datalen;
	
	outname = replace_ext(filename, ".bmp", ".img");
	if (outname == NULL)
	{
		oom();
		goto error;
	...
	if ((long) fwrite(buf, 1, headerlen, out) != headerlen ||
		(long) fwrite(buf + pic.pi_dataoffset, 1, datalen, out) != datalen ||
		fflush(out) != 0 ||
		ferror(out))
	{
