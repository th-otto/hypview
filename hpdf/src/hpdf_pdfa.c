 ...
 */
static HPDF_STATUS ConvertDateToXMDate(HPDF_Stream stream, const char *pDate)
{
	HPDF_STATUS ret;
...
HPDF_STATUS HPDF_PDFA_SetPDFAConformance(HPDF_Doc pdf, HPDF_PDFAType pdfatype)
{
	...
	const char *dc_title;
	const char *dc_creator;
	const char *dc_description;
	...
	if (dc_title != NULL || dc_creator != NULL || dc_description != NULL ||
		xmp_CreateDate != NULL || xmp_ModifyDate != NULL || xmp_CreatorTool != NULL || pdf_Keywords != NULL)
	{
...
HPDF_STATUS HPDF_PDFA_GenerateID(HPDF_Doc pdf)
{
	...
	if (!id)
	{
		...
		HPDF_MD5Update(&md5_ctx, (const HPDF_BYTE *) "libHaru", sizeof("libHaru") - 1);
