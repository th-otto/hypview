	...
	while (sym)
	{
		if (sym->lineno == lineno)
		{
			...
			if (sym->from_idx && !sym->from_ref)
				...
				hyp_utf8_sprintf_charset(out, opts->output_charset, "    <label>%s</label><!-- lineno %u -->\n", str, sym->lineno);
			g_free(str);
			sym->referenced = TRUE;
		}
...
#define FLUSHTREE() \
	...
	entry = hyp->indextable[node];
	hyp_utf8_sprintf_charset(out, opts->output_charset, "  <node index=\"%u\"%s>\n",
		node,
		entry->type == HYP_NODE_INTERNAL ? "" : " type=\"popup\"");
...
static gboolean recompile_xml(HYP_DOCUMENT *hyp, hcp_opts *opts, int argc, const char **argv)
{
	...
	g_string_append(out, "<!DOCTYPE resource SYSTEM \"http://somewhere/resource.dtd\" [\n");
	g_string_append(out, "<!ENTITY escape \"&#9243;\">\n");
