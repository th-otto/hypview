...
static void CleanFunc(HPDF_FontDef fontdef)
{
	...
	memset(attr->glyph_tbl.flgs, 0, sizeof(HPDF_BYTE) * attr->num_glyphs);
...
static HPDF_STATUS LoadTTFTable(HPDF_FontDef fontdef)
{
	...
	attr->offset_tbl.table = (HPDF_TTFTable *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_TTFTable) * attr->offset_tbl.num_tables);
	if (!attr->offset_tbl.table)
		return HPDF_Error_GetCode(fontdef->error);
	...
	for (i = 0; i < attr->offset_tbl.num_tables; i++)
	{
		...
		HPDF_PTRACE((" [%d] tag=[%c%c%c%c] check_sum=%u offset=%u length=%u\n",
					 ...
					 (unsigned int) tbl->check_sum, (unsigned int) tbl->offset, (unsigned int) tbl->length));
...
static HPDF_STATUS ParseCMAP_format4(HPDF_FontDef fontdef, HPDF_UINT32 offset)
{
	...
	/* end_count */
	attr->cmap.end_count = (HPDF_UINT16 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
	if (!attr->cmap.end_count)
		return HPDF_Error_GetCode(fontdef->error);
	...
	/* start_count */
	attr->cmap.start_count = (HPDF_UINT16 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
	if (!attr->cmap.start_count)
		return HPDF_Error_GetCode(fontdef->error);
	...
	/* id_delta */
	attr->cmap.id_delta = (HPDF_INT16 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
	if (!attr->cmap.id_delta)
		return HPDF_Error_GetCode(fontdef->error);
	...
	/* id_range_offset */
	attr->cmap.id_range_offset = (HPDF_UINT16 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
	if (!attr->cmap.id_range_offset)
		return HPDF_Error_GetCode(fontdef->error);
	...
	{
		/* glyph_id_array */
		attr->cmap.glyph_id_array = (HPDF_UINT16 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.glyph_id_array_count);
		if (!attr->cmap.glyph_id_array)
			return HPDF_Error_GetCode(fontdef->error);
...
static HPDF_STATUS ParseCMAP_format0(HPDF_FontDef fontdef, HPDF_UINT32 offset)
{
	...
	attr->cmap.glyph_id_array = (HPDF_UINT16 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * 256);
	if (!attr->cmap.glyph_id_array)
		return HPDF_Error_GetCode(fontdef->error);
...
static HPDF_STATUS ParseCMap(HPDF_FontDef fontdef)
{
	...
	for (i = 0; i < num_cmap; i++)
	{
		...
		HPDF_PTRACE((" HPDF_TTFontDef_ParseCMap tables[%d] platformID=%u "
					 "encodingID=%u format=%u offset=%u\n", i, platformID, encodingID, format, (HPDF_UINT) offset));
...
static HPDF_STATUS ParseLoca(HPDF_FontDef fontdef)
{
	...
	attr->glyph_tbl.offsets = (HPDF_UINT32 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT32) * (attr->num_glyphs + 1));

	if (!attr->glyph_tbl.offsets)
		return HPDF_Error_GetCode(fontdef->error);

	memset(attr->glyph_tbl.offsets, 0, sizeof(HPDF_UINT32) * (attr->num_glyphs + 1));

	/* allocate glyph-flg-table.
	 * this flgs are used to judge whether glyphs should be embedded.
	 */
	attr->glyph_tbl.flgs = (HPDF_BYTE *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_BYTE) * attr->num_glyphs);
...
static HPDF_STATUS ParseHmtx(HPDF_FontDef fontdef)
{
	 ...
	 */
	attr->h_metric = (HPDF_TTF_LongHorMetric *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_TTF_LongHorMetric) * attr->num_glyphs);
...
static HPDF_STATUS LoadUnicodeName(HPDF_Stream stream, HPDF_UINT offset, HPDF_UINT len, char *buf)
{
	...
	memset(buf, 0, HPDF_LIMIT_MAX_NAME_LEN + 1);
...
static HPDF_STATUS ParseName(HPDF_FontDef fontdef)
{
	...
	HPDF_PTRACE((" ParseName() format=%u, count=%u, string_offset=%u\n",
				 attr->name_tbl.format, attr->name_tbl.count, attr->name_tbl.string_offset));

	attr->name_tbl.name_records = (HPDF_TTF_NameRecord *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_TTF_NameRecord) * attr->name_tbl.count);
	...
	for (i = 0; i < attr->name_tbl.count; i++)
	{
		...
		HPDF_PTRACE((" ParseName() platformID=%u, encodingID=%d, nameID=%d\n",
					 name_rec->platform_id, name_rec->encoding_id, name_rec->name_id));
	...
	memset(attr->base_font, 0, HPDF_LIMIT_MAX_NAME_LEN + 1);
	...
	memset(tmp, 0, HPDF_LIMIT_MAX_NAME_LEN + 1);
	...
	memcpy(fontdef->base_font, attr->base_font, HPDF_LIMIT_MAX_NAME_LEN + 1);
...
static HPDF_STATUS ParseOS2(HPDF_FontDef fontdef)
{
	...
	HPDF_PTRACE(("  ParseOS2 CodePageRange1=%08X CodePageRange2=%08X\n",
				 (unsigned int) attr->code_page_range1, (unsigned int) attr->code_page_range2));
...
static void DumpTable(HPDF_FontDef fontdef)
{
	...
	for (i = 0; i < HPDF_REQUIRED_TAGS_COUNT; i++)
	{
		...
		memset(fname, 0, 9);
		memcpy(fname, REQUIRED_TAGS[i], 4);
		memcpy(fname + 4, ".dat", 4);
		HPDF_PTRACE((" %s open\n", fname));

		if (memcmp(fname, "OS/2", 4) == 0)
			fname[2] = '_';
...
HPDF_Box HPDF_TTFontDef_GetCharBBox(HPDF_FontDef fontdef, HPDF_UINT16 unicode)
{
	...
	HPDF_PTRACE((" PdfTTFontDef_GetCharBBox char=0x%04X, "
				 "box=[%f,%f,%f,%f]\n", unicode, bbox.left, bbox.bottom, bbox.right, bbox.top));
...
HPDF_UINT16 HPDF_TTFontDef_GetGlyphid(HPDF_FontDef fontdef, HPDF_UINT16 unicode)
{
	...
	if (attr->cmap.id_range_offset[i] == 0)
	{
		HPDF_PTRACE((" HPDF_TTFontDef_GetGlyphid idx=%u code=%u "
					 " ret=%u\n", i, unicode, unicode + attr->cmap.id_delta[i]));
	...
	} else
	{
		...
		if (idx > attr->cmap.glyph_id_array_count)
		{
			HPDF_PTRACE((" HPDF_TTFontDef_GetGlyphid[%u] %u > %u\n",
						 i, idx, (unsigned int) attr->cmap.glyph_id_array_count));
			return 0;
		} else
		{
			HPDF_UINT16 gid = (HPDF_UINT16) (attr->cmap.glyph_id_array[idx] + attr->cmap.id_delta[i]);

			HPDF_PTRACE((" HPDF_TTFontDef_GetGlyphid idx=%u unicode=0x%04X " "id=%u\n", idx, unicode, gid));
			return gid;
		}
	}
}
	...
	} else
	{
		...
		do
		{
			...
			HPDF_PTRACE((" gid=%d, num_of_contours=%d, flags=%d, "
						 "glyph_index=%d\n", gid, num_of_contours, flags, glyph_index));
...
HPDF_INT16 HPDF_TTFontDef_GetCharWidth(HPDF_FontDef fontdef, HPDF_UINT16 unicode)
{
	...
	{
		HPDF_PTRACE((" HPDF_TTFontDef_GetCharWidth WARNING gid > " "num_glyphs %u > %u\n", gid, attr->num_glyphs));
...
HPDF_INT16 HPDF_TTFontDef_GetGidWidth(HPDF_FontDef fontdef, HPDF_UINT16 gid)
{
	...
	if (gid >= attr->num_glyphs)
	{
		HPDF_PTRACE((" HPDF_TTFontDef_GetGidWidth WARNING gid > " "num_glyphs %u > %u\n", gid, attr->num_glyphs));
		return fontdef->missing_width;
	}
	...
	for (i = 0; i < attr->num_glyphs; i++)
	{
		...
		if (attr->glyph_tbl.flgs[i] == 1)
		{
			...
			HPDF_PTRACE((" RecreateGLYF[%u] move from [%u] to [%u]\n", i,
						 (unsigned int) attr->glyph_tbl.base_offset + offset, (unsigned int) new_offsets[i]));
...
static HPDF_STATUS RecreateName(HPDF_FontDef fontdef, HPDF_Stream stream)
{
	...
	for (i = 0; i < attr->name_tbl.count; i++)
	{
		...
		HPDF_PTRACE((" RecreateNAME name_rec[%u] platform_id=%u "
					 ...
					 name_rec->encoding_id, name_rec->language_id, name_rec->name_id, name_len, rec_offset));
...
HPDF_STATUS HPDF_TTFontDef_SaveFontData(HPDF_FontDef fontdef, HPDF_Stream stream)
{
	...
	new_offsets = (HPDF_UINT32 *) HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT32) * (attr->num_glyphs + 1));
	if (!new_offsets)
	{
		HPDF_Stream_Free(tmp_stream);
		return HPDF_Error_GetCode(fontdef->error);
	...
	for (i = 0; i < HPDF_REQUIRED_TAGS_COUNT; i++)
	{
		...
		{
			tbl = &emptyTable;
			memcpy(tbl->tag, REQUIRED_TAGS[i], 4);
		}
		...
		} else if (memcmp(tbl->tag, "name", 4) == 0)
		{
...
void HPDF_TTFontDef_SetTagName(HPDF_FontDef fontdef, char *tag)
{
	...
	memcpy(attr->tag_name, tag, HPDF_TTF_FONT_TAG_LEN);
	...
	memset(buf, 0, HPDF_LIMIT_MAX_NAME_LEN + 1);
	memcpy(buf, attr->tag_name, HPDF_TTF_FONT_TAG_LEN + 1);
	memcpy(buf + HPDF_TTF_FONT_TAG_LEN + 1, fontdef->base_font, HPDF_LIMIT_MAX_NAME_LEN - HPDF_TTF_FONT_TAG_LEN - 1);

	memcpy(attr->base_font, buf, HPDF_LIMIT_MAX_NAME_LEN + 1);
}
