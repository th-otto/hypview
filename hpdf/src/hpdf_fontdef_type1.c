         ...
         HPDF_Stream   stream)
{
    ...
    if (strcmp (keyword, "StartFontMetrics") != 0)
    ...
    memset (cdata, 0, sizeof(HPDF_CharData) * attr->widths_count);
    attr->widths = cdata;
                              ...
                              const HPDF_CharData*  widths)
{
    ...
    memset (dst, 0, sizeof(HPDF_CharData) * attr->widths_count);
    attr->widths = dst;
